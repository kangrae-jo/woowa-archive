...
- 로그는 Lombok `@Slf4j`가 생성한 SLF4J logger로 기록하고 `slf4j-simple`을 콘솔 출력 provider로 사용한다. `{}` 매개변수 로깅과 마지막 Throwable 인자로 식별 정보·원인·스택 트레이스를 보존한다. API와 provider 버전은 동일 SLF4J BOM으로 맞춘다.
