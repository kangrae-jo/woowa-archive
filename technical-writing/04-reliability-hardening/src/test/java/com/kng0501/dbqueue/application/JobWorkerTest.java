...
class JobWorkerTest {
    ...
    void Generator는_선점_커밋_후_DB_트랜잭션_없이_실행한다() {
        Job claim = queue.tryClaim(queue.request("dragon").jobId()).orElseThrow();
        var worker = new JobWorker(queue, prompt -> {
    ...
    void 실패를_DB에_기록하지_못해도_RUNNING을_유지해_기한_복구할_수_있다() {
        ...
        Job running = queue.findJob(claim.jobId()).orElseThrow();
        ...
        Job retry = queue.tryClaim(claim.jobId()).orElseThrow();
    ...
    void 한_Job의_복구_저장_실패가_다른_만료_Job의_복구를_막지_않는다() {
        Job first = queue.tryClaim(queue.request("first").jobId()).orElseThrow();
        Job second = queue.tryClaim(queue.request("second").jobId()).orElseThrow();
