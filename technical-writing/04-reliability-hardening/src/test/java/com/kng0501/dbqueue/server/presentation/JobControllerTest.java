...
final class JobControllerTest {
    ...
    @Autowired
    JobControllerTest(final MockMvc mockMvc, final JdbcTemplate jdbc, final ApplicationContext context) {
    ...
    @Test
    void 서버는_등록과_상태_조회만_제공한다() throws Exception {
        ...
        assertTrue(context.getBeansOfType(Object.class).values().stream()
                .map(Object::getClass)
                .noneMatch(type -> type.getName().startsWith("com.kng0501.dbqueue.worker.")));
    }
}
