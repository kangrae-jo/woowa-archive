...
2025년 당시 약 300건의 이미지 생성 요청을 처리하던 중 시스템이 사용자 요청에 응답하지 못하는 장애를 경험했다. 이후 이미지 생성 대상을 별도의 `Prompt` 테이블로 분리하고, 연속 요청과 생성 개수를 제한했다. 최종 발표에서는 이전과 비슷한 규모의 요청을 처리했다. 따라서 당시 운영 환경에서는 시스템 안정화에 도움이 된 대응으로 판단했다.
...
- 약 300건의 이미지 생성 요청을 처리하던 중 시스템이 사용자 요청에 응답하지 못하는 장애를 경험했다.
- Monster 탐색 대신 `Prompt` 테이블에 이미지 생성 작업을 저장하고, 연속 요청·생성 개수 제한을 함께 적용했다.
...
- 당시 요청 미응답 장애의 단일 원인은 확인하지 못했다.

## 두 종류의 변경을 구분한다

| 구분 | 범위 | 이 문서에서 내릴 수 있는 판단 |
| --- | --- | --- |
| 2025년 당시 안정화 | `Prompt` 테이블 분리, 연속 요청 제한, 생성 개수 제한 | 함께 적용한 뒤 비슷한 행사를 운영. 개별 기여도는 확인 불가 |
| 현재 신뢰성 개선 | 작업 상태, 원자적 선점, 처리 타임아웃 복구, 재시도, 멱등한 결과 반영 | 04단계에 구현. MySQL 통합 검증은 확인 필요 |
